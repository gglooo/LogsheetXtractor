...
public class OcrCredentialService(IConfiguration config) : IOcrCredentialService
{
    private readonly string _googleCredentialsPath =
        config["Credentials:GoogleApiKeyPath"] ?? string.Empty;
    private readonly string _azureCredentialsPath =
        config["Credentials:AzureApiKeyPath"] ?? string.Empty;
    private readonly string _amazonCredentialsPath =
        config["Credentials:AmazonApiKeyPath"] ?? string.Empty;

    private Dictionary<ECredentialType, string> CredentialPaths =>
        new()
        {
    ...
    {
        if (
            !CredentialPaths.TryGetValue(credentialType, out var path) || string.IsNullOrEmpty(path)
        )
        {
            return null;
        }
