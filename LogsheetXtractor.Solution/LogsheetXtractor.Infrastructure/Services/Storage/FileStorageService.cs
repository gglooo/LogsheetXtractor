...
namespace LogsheetXtractor.Infrastructure.Services.Storage;

public class FileStorageService(IConfiguration config) : IFileStorageService
{
    private readonly string _storageDirectory = Path.GetFullPath(
    ...
    public async Task<string> SaveTemporaryFileAsync(
        byte[] fileData,
        string fileName,
