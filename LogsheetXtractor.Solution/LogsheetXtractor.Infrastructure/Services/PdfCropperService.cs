...
) : IPdfCropperService
{
    ...
    public PdfDimensionsDto GetPageDimensions(byte[] bytes, int pageNumber, CancellationToken ct)
    {
        ...
        return new PdfDimensionsDto { Width = width, Height = height };
    }

    public Result<Stream> GetCroppedSection(
        byte[] bytes,
        int pageNumber,
        int x,
        int y,
    ...
    )
    {
        ...
        if (cropRect.IsEmpty)
        {
            ...
            return Result.Fail(
                new InvalidStateError("Crop rectangle is empty or outside image bounds.")
            );
        }
    ...
    public Result<Stream> GetWarpedSection(
        byte[] bytes,
        int pageNumber,
    ...
    )
    {
        ...
        canvas.DrawBitmap(
            ...
            0,
            0,
            new SKPaint { IsAntialias = true, FilterQuality = SKFilterQuality.Medium }
        );
