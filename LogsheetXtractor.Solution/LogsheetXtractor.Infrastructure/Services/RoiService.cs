    ...
    : IRoiService
{
    ...
    )
    {
        ...
        var existingRois = await dbContext
            .Rois.Include(r => r.Coordinates)
        ...
        var roisToDelete = existingRois.Values.Where(r => !incomingGuids.Contains(r.Id)).ToList();
    ...
    public async Task<Result<IEnumerable<RoiDto>>> CloneRoisForTemplateAsync(
        Guid sourceTemplateId,
        Guid targetTemplateId,
        CancellationToken cancellationToken
    )
    {
        var sourceRois = await dbContext
            .Rois.AsNoTracking()
            .Where(r => r.TemplateId == sourceTemplateId)
            .ToListAsync(cancellationToken);

        var clonedRois = sourceRois
            .Select(r =>
            {
