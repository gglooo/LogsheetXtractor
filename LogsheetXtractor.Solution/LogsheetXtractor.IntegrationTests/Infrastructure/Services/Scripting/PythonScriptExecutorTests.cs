...
public class PythonScriptExecutorTests
{
    ...
    private readonly Mock<Microsoft.Extensions.Logging.ILogger<PythonScriptExecutor>> _loggerMock =
        new();
    ...
    public async Task ExecuteScriptWithJsonOutputAsync_ShouldEnsureDeserialization_WhenOutputIsValid()
    {
        ...
        _serviceMock
            .Setup(x => x.ExecuteScriptAsync(scriptName, args, It.IsAny<CancellationToken>()))
            .ReturnsAsync(jsonOutput);

        var result = await _serviceMock.Object.ExecuteScriptWithJsonOutputAsync<TestDto>(
            scriptName,
            args,
    ...
    public async Task ExecuteScriptWithJsonOutputAsync_ShouldThrow_WhenOutputIsInvalidJson()
    {
        ...
        Func<Task> act = async () =>
            await _serviceMock.Object.ExecuteScriptWithJsonOutputAsync<TestDto>(
                scriptName,
                args,
                CancellationToken.None
            );
