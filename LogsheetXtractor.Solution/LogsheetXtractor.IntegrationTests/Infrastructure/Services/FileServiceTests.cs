...
public class FileServiceTests : IDisposable
{
    ...
    public async Task UploadFileAsync_ShouldSaveFileToDiskAndDb()
    {
        ...
        _mapperMock
            .Setup(x => x.Map<FileDto>(It.IsAny<LogsheetXtractor.Domain.Entities.File>()))
            .Returns(expectedDto);

        _fileStorageServiceMock
            .Setup(x => x.SaveFileAsync(content, fileName))
    ...
    public async Task GetFileAsync_ShouldReturnFile_WhenExists()
    {
        ...
        var fileEntity = new LogsheetXtractor.Domain.Entities.File
        {
            Id = Guid.NewGuid(),
        ...
        _fileStorageServiceMock.Setup(x => x.GetFile(storagePath)).Returns(fileStream);
