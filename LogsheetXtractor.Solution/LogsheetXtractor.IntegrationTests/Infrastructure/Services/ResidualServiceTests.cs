...
public class ResidualServiceTests : IDisposable
{
    ...
    public async Task SetResidualsForTemplateAsync_ShouldCreateNewResiduals_WhenIdsAreEmpty()
    {
        ...
        _mapperMock
            .Setup(m => m.Map<Residual>(It.IsAny<SetResidualDto>()))
            .Returns(
                (SetResidualDto dto) =>
                    new Residual
                    {
        ...
        // Act
        var result = await _sut.SetResidualsForTemplateAsync(
            templateId,
            updateResiduals,
    ...
    public async Task SetResidualsForTemplateAsync_ShouldUpdateExistingResiduals_WhenIdsAreProvided()
    {
        ...
        _mapperMock
            .Setup(m => m.Map(It.IsAny<SetResidualDto>(), It.IsAny<Residual>()))
            .Callback(
                (SetResidualDto dto, Residual residual) =>
                {
    ...
    public async Task SetResidualsForTemplateAsync_ShouldDeleteResiduals_WhenNotIncludedInUpdate()
    {
        ...
        // Act
        await _sut.SetResidualsForTemplateAsync(
            templateId,
            updateResiduals,
            CancellationToken.None
        );
