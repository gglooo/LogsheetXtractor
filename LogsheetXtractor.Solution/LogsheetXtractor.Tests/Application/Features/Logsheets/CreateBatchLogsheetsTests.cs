...
public class CreateBatchLogsheetsTests : IDisposable
{
    ...
    public async Task Handle_ShouldReturnEmptyList_WhenFileIdsListIsEmpty()
    {
        ...
        var result = await CreateBatchLogsheetsHandler.Handle(
            command,
    ...
    public async Task Handle_ShouldFail_WhenAnyFileAlreadyAssigned()
    {
        ...
        result
            .Errors.Should()
            .Contain(e => e.Message == "One or more files are already assigned to a logsheet");
    }
    ...
    public async Task Handle_ShouldCreateLogsheets_WhenRequestIsValid()
    {
        ...
        _mapperMock
            .Setup(m => m.Map<IList<Logsheet>>(It.IsAny<IEnumerable<CreateLogsheetCommand>>()))
            .Returns(new List<Logsheet> { logsheet1, logsheet2 });

        var expectedDtos = new List<LogsheetDetailDto>();
        _mapperMock
            .Setup(m => m.Map<IEnumerable<LogsheetDetailDto>>(It.IsAny<List<Logsheet>>()))
    ...
    public async Task Handle_ShouldCreateLogsheets_WhenRequestIsValid_WithBackside()
    {
        ...
        var command = new BatchCreateLogsheetCommand(
            templateId,
            backsideTemplateId,
            new[] { fileId }
        );
    ...
    private LogsheetXtractor.Domain.Entities.File newDomainFile(Guid id)
    {
