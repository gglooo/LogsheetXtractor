...
public class ListLogsheetsByTemplateQueryHandlerTests : IDisposable
{
    ...
    public async Task Handle_ShouldReturnLogsheets_ForGivenTemplate()
    {
        ...
        _mapperMock
            .Setup(x => x.Map<IEnumerable<LogsheetListDto>>(It.IsAny<IEnumerable<Logsheet>>()))
            .Returns(
                (IEnumerable<Logsheet> logsheets) =>
                {
                    var list = logsheets.ToList();
                    if (
                        list.Count == 2
                        && list.Any(l => l.Id == logsheet1.Id)
                        && list.Any(l => l.Id == logsheet2.Id)
                    )
                    {
                        return expectedDtos;
                    }
        ...
        var result = await ListLogsheetsByTemplateHandler.Handle(
            query,
            _dbContext,
            _mapperMock.Object
        );
