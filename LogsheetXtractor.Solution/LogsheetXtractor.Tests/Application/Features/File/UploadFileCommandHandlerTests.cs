...
namespace LogsheetXtractor.Tests.Application.Features.File;
...
public class UploadFileCommandHandlerTests
{
    ...
    public async Task Handle_ShouldUploadFileAndSaveChanges_WhenSuccessful()
    {
        ...
        var expectedFileDto = new FileDto(
            Guid.NewGuid(),
            "test.txt",
            "text/plain",
            3u,
        ...
        _fileServiceMock
            .Setup(x =>
                x.UploadFileAsync(command.FileContent, command.FileName, command.ContentType)
            )
            .ReturnsAsync(expectedFileDto);

        var result = await UploadFileHandler.Handle(
            command,
