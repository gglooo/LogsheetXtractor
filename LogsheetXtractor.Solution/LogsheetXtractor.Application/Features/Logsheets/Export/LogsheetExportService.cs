...
) : ILogsheetExportService
{
    ...
    )
    {
        ...
        exportData.AddRange(
            logsheet.ExtractedValues.Select(extractedValue => new ExportLogsheetDataDto
            {
        ...
        try
        {
            var fileResult = await scriptEngine.ExportLogsheetDataAsync(
                logsheet,
                exportData,
                logsheet.File,
                logsheet.Template.File,
                ct
            );
            return fileResult;
        }
                    ...
                    foreach (var logsheetId in logsheetIds)
                    {
                        ...
                        if (result.IsFailed)
                        {
                            return Result.Fail<GetFileDto>(
                                $"Failed to export logsheet {logsheetId}: {result.Errors.First().Message}"
                            );
                        }
