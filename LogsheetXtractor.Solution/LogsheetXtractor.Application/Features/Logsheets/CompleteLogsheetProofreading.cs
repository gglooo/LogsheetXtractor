...
public sealed record CompleteLogsheetProofreadingCommand(Guid LogsheetId);
...
public static class CompleteLogsheetProofreadingHandler
{
    ...
    )
    {
        logger.LogInformation(
            "Completing proofreading for Logsheet {LogsheetId}",
            request.LogsheetId
        );

        var logsheet = dbContext
            .Logsheets.Include(l => l.ExtractedValues)
        ...
        var areAllValuesVerified = dbContext
            .ExtractedValues.Where(ev => ev.LogsheetId == logsheet.Id)
        ...
        if (logsheet.Status != ELogSheetStatus.NeedsReview || !areAllValuesVerified)
        {
            ...
            return Result.Fail(
                new InvalidStateError(
                    "Logsheet is not in a state that allows completing proofreading."
                )
            );
        }
        ...
        logger.LogInformation(
            "Proofreading completed successfully for Logsheet {LogsheetId}",
            request.LogsheetId
        );
