...
public static class CreateBatchLogsheetsHandler
{
    ...
    )
    {
        ...
        logger.LogInformation(
            "Starting batch logsheet creation for {Count} files with Template {TemplateId}",
            fileIds.Count,
            request.TemplateId
        );
        ...
        var files = await dbContext
            .Files.Where(f => fileIds.Contains(f.Id))
            .Select(f => f.Id)
            .ToListAsync(ct);
        ...
        var existingAssignments = await dbContext
            .Logsheets.Where(l => fileIds.Contains(l.FileId))
            .AnyAsync(ct);
        ...
        if (existingAssignments)
        {
            ...
            return Result.Fail(
                new ConstraintError("One or more files are already assigned to a logsheet")
            );
        }

        var templates = await dbContext
            .Templates.Include(t => t.Rois)
        ...
        var logsheets = mapper.Map<IList<Logsheet>>(
            request.FileIds.Select(
                (fileId) =>
        ...
        foreach (var createdLogsheet in logsheets)
        {
            await bus.PublishAsync(
                new LogsheetCreatedEvent(createdLogsheet.Id, request.PerformAutomaticAlignment)
            );
        }

        var newIds = logsheets.Select(x => x.Id).ToList();

        var resultEntities = await dbContext
