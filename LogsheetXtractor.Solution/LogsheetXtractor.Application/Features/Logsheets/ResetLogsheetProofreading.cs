...
public sealed record ResetLogsheetProofreadingCommand(Guid LogsheetId);
...
public static class ResetLogsheetProofreadingHandler
{
    ...
    )
    {
        logger.LogInformation(
            "Resetting proofreading for Logsheet {LogsheetId}",
            request.LogsheetId
        );
        ...
        {
            logger.LogWarning(
                "Cannot reset proofreading for Logsheet {LogsheetId} because it is currently processing.",
                request.LogsheetId
            );
            return Result.Fail(
                new InvalidStateError(
                    "Logsheet must finish processing before resetting proofreading."
                )
            );
        }
        ...
        logger.LogInformation(
            "Proofreading reset successfully for Logsheet {LogsheetId}",
            request.LogsheetId
        );
        return new Result();
    }
