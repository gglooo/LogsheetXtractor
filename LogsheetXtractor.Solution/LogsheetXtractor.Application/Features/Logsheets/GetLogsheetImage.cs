...
public static class GetLogsheetImageHandler
{
    ...
    )
    {
        ...
        var backsideTemplate = !query.IsFrontside
            ? await dbContext
                ...
                .FirstOrDefaultAsync(
                    t => t.Id == logsheet.Template.BacksideTemplateId,
                    cancellationToken
                )
            : null;
        ...
        var template = query.IsFrontside ? logsheet.Template : backsideTemplate;
        ...
        var warpedStreamResult = pdfCropperService.GetWarpedSection(
            pdfBytes,
            page,
            srcPoints,
            dstPoints,
            template.Width * outputScale ?? 0,
            template.Height * outputScale ?? 0,
            template.Width ?? 0,
            template.Height ?? 0,
            cancellationToken
        );
    ...
    private static List<PointCoordinate> GetTemplateCorners(
