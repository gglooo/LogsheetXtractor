    ...
    : IRoiValidationConditionTreeValidator
{
    public Result Validate(ERoiType roiType, RoiValidationConditionNode conditionRoot)
    {
        ...
        {
            return Result.Fail(
                new ValidationError("Validation condition root must be a group node.")
            );
        }
    ...
    )
    {
        ...
        {
            if (
                !string.Equals(node.Operator, "AND", StringComparison.OrdinalIgnoreCase)
            ...
            {
                return Result.Fail(
                    new ValidationError($"Group at '{path}' must have at least one child.")
                );
            }
            ...
            {
                return Result.Fail(
                    new ValidationError($"Schema version can be set only at root. Path '{path}'.")
                );
            }
        ...
        if (string.Equals(node.Type, "rule", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                return Result.Fail(
                    new ValidationError($"Rule at '{path}' must define 'ruleType'.")
                );
            }
            ...
            {
                return Result.Fail(
                    new ValidationError($"Rule at '{path}' must not define children.")
                );
            }
            ...
            {
                return Result.Fail(
                    new ValidationError($"Unsupported rule '{node.RuleType}' at '{path}'.")
                );
            }
            ...
            {
                var message =
                    paramsValidation.Errors.FirstOrDefault()?.Message ?? "Invalid rule parameters.";
        ...
        return Result.Fail(
            new ValidationError($"Unsupported node type '{node.Type}' at '{path}'.")
        );
    }
}
