...
public sealed class TextPrefixRuleEvaluator : RoiValidationRuleEvaluatorBase
{
    ...
    public override IReadOnlySet<ERoiType> SupportedRoiTypes =>
        new HashSet<ERoiType> { ERoiType.Barcode };
    public override JsonElement DefaultParams =>
        RoiValidationJson.ParseDefault("{\"prefix\":\"\"}");
    ...
    )
    {
        ...
        {
            return Result.Fail(
                new RoiValidationWarningError(RuleType, "Invalid text.prefix parameters.", path)
            );
        }
        ...
        return input.StartsWith(parsed.Value.Prefix, StringComparison.Ordinal)
            ...
            : Result.Fail(
                new RoiValidationWarningError(
                    RuleType,
                    $"Text must start with '{parsed.Value.Prefix}'.",
                    path
                )
            );
    }
