...
file sealed record NumberRangeParams(
    decimal? Min,
    decimal? Max,
...
public sealed class NumberRangeRuleEvaluator : RoiValidationRuleEvaluatorBase
{
    ...
    public override JsonElement DefaultParams =>
        RoiValidationJson.ParseDefault(
            "{\"min\":0,\"max\":100,\"inclusiveMin\":true,\"inclusiveMax\":true}"
        );
    ...
    public override Result ValidateParams(JsonElement? parameters)
    {
        ...
        {
            return Result.Fail(
                new ValidationError("number.range requires at least one boundary (min or max).")
            );
        }
        ...
        {
            return Result.Fail(
                new ValidationError("number.range min must be less than or equal to max.")
            );
        }
    ...
    )
    {
        ...
        {
            return Result.Fail(
                new RoiValidationWarningError(RuleType, "Invalid number.range parameters.", path)
            );
        }
            ...
            if (!minOk)
            {
                return Result.Fail(
                    new RoiValidationWarningError(
                        RuleType,
                        $"Value must be greater than {(p.InclusiveMin ? "or equal to " : string.Empty)}{p.Min}.",
                        path
                    )
                );
            }
        ...
        if (p.Max is not null)
        {
            ...
            if (!maxOk)
            {
                return Result.Fail(
                    new RoiValidationWarningError(
                        RuleType,
                        $"Value must be less than {(p.InclusiveMax ? "or equal to " : string.Empty)}{p.Max}.",
                        path
                    )
                );
            }
