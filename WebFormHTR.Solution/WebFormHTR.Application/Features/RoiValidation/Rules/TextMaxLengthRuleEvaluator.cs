...
public sealed class TextMaxLengthRuleEvaluator : RoiValidationRuleEvaluatorBase
{
    ...
    public override Result Evaluate(RoiValidationRuleEvaluationContext context, JsonElement? parameters, string path)
    {
        ...
        if (parsed.IsFailed)
        {
            return Result.Fail(new RoiValidationWarningError(RuleType, "Invalid text.maxLength parameters.", path));
        }

        var normalized = RoiValidationValueParsing.NormalizeText(context.Value);
        return normalized.Length <= parsed.Value.Max
            ? Result.Ok()
            : Result.Fail(new RoiValidationWarningError(RuleType, $"Text must be at most {parsed.Value.Max} characters long.", path));
    }
}
