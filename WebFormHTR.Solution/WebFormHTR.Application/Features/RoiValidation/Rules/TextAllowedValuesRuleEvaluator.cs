...
public sealed class TextAllowedValuesRuleEvaluator : RoiValidationRuleEvaluatorBase
{
    ...
    public override IReadOnlySet<ERoiType> SupportedRoiTypes => new HashSet<ERoiType> { ERoiType.Handwritten };
    ...
    public override Result ValidateParams(JsonElement? parameters)
    {
        ...
        return parsed.Value.Values.Count == 0
            ? Result.Fail(new ValidationError("text.allowedValues requires a non-empty values list."))
            : Result.Ok();
    }
    ...
    public override Result Evaluate(RoiValidationRuleEvaluationContext context, JsonElement? parameters, string path)
    {
        ...
        if (parsed.IsFailed)
        {
            return Result.Fail(new RoiValidationWarningError(RuleType, "Invalid text.allowedValues parameters.", path));
        }
        ...
        return parsed.Value.Values.Contains(input, StringComparer.Ordinal)
            ? Result.Ok()
            : Result.Fail(new RoiValidationWarningError(RuleType, "Text is not in allowed values.", path));
    }
}
