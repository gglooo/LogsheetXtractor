...
public sealed class TextRegexRuleEvaluator : RoiValidationRuleEvaluatorBase
{
    ...
    public override Result Evaluate(RoiValidationRuleEvaluationContext context, JsonElement? parameters, string path)
    {
        ...
        {
            return Result.Fail(new RoiValidationWarningError(RuleType, "Invalid text.regex parameters.", path));
        }
        ...
        try
        {
            ...
            return regex.IsMatch(input)
                ? Result.Ok()
                : Result.Fail(new RoiValidationWarningError(RuleType, "Text does not match required pattern.", path));
        }
        catch
        {
