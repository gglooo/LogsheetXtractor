...
internal static class RoiValidationJson
{
    ...
    public static Result<T> Deserialize<T>(JsonElement? parameters)
    {
        try
        {
            var json = parameters is null || parameters.Value.ValueKind is JsonValueKind.Null or JsonValueKind.Undefined
                ? "{}"
            ...
            if (parsed is null)
            {
                return Result.Fail<T>(new ValidationError("Rule parameters are missing or invalid."));
            }
        ...
        {
            return Result.Fail<T>(new ValidationError($"Failed to parse rule parameters: {ex.Message}"));
        }
    }
}
...
internal static class RoiValidationValueParsing
{
    ...
    public static bool TryParseNumber(string? input, out decimal value)
    {
        ...
        return decimal.TryParse(withDotSeparator, NumberStyles.Number, CultureInfo.InvariantCulture, out value);
    }
