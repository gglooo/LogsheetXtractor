...
public sealed class NumberNotInSetRuleEvaluator : RoiValidationRuleEvaluatorBase
{
        ...
        string path)
    {
        ...
        if (parsed.IsFailed)
        {
            return Result.Fail(new RoiValidationWarningError(RuleType, "Invalid number.notInSet parameters.", path));
        }
        ...
        {
            if (RoiValidationValueParsing.TryParseNumber(raw, out var disallowed) && number == disallowed)
            {
                return Result.Fail(new RoiValidationWarningError(RuleType, "Value is disallowed.", path));
            }
        }
