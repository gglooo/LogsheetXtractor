...
public sealed class TextNotRegexRuleEvaluator : RoiValidationRuleEvaluatorBase
{
    ...
    public override JsonElement DefaultParams => RoiValidationJson.ParseDefault("{\"pattern\":\"\"}");
    ...
    public override Result ValidateParams(JsonElement? parameters)
    {
        ...
        {
            return Result.Fail(new ValidationError($"text.notRegex pattern is invalid: {ex.Message}"));
        }
    }
    ...
    public override Result Evaluate(RoiValidationRuleEvaluationContext context, JsonElement? parameters, string path)
    {
        ...
        {
            return Result.Fail(new RoiValidationWarningError(RuleType, "Invalid text.notRegex parameters.", path));
        }

        var input = context.Value ?? string.Empty;
        if (parsed.Value.MaxInputLength is not null && input.Length > parsed.Value.MaxInputLength)
        {
            return Result.Fail(new RoiValidationWarningError(RuleType, $"Text is too long for regex evaluation (>{parsed.Value.MaxInputLength}).", path));
        }
        ...
        try
        {
            ...
            return regex.IsMatch(input)
                ? Result.Fail(new RoiValidationWarningError(RuleType, "Text matches forbidden pattern.", path))
                : Result.Ok();
        }
        catch
        {
            return Result.Fail(new RoiValidationWarningError(RuleType, "Regex evaluation failed.", path));
        }
    }
}
