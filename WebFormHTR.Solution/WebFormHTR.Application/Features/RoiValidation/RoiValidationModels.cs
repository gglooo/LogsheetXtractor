...
public sealed record RoiValidationRuleEvaluationContext(
    ERoiType RoiType,
    string? Value
);
...
{
    public RoiValidationWarningError(string code, string message, string path) : base(message)
    {
...
{
    Result Validate(ERoiType roiType, Domain.ValueObjects.RoiValidation.RoiValidationConditionNode conditionRoot);
}
...
public interface IRoiValidationConditionEvaluator
{
    IReadOnlyList<RoiValidationWarningDto> Evaluate(
        ERoiType roiType,
        string? value,
        Domain.ValueObjects.RoiValidation.RoiValidationConditionNode? conditionRoot);
}

public interface IRoiValidationConditionNodeEvaluator
{
    bool CanHandle(Domain.ValueObjects.RoiValidation.RoiValidationConditionNode node);

    Result Evaluate(
        Domain.ValueObjects.RoiValidation.RoiValidationConditionNode node,
        RoiValidationRuleEvaluationContext context,
        string path,
        Func<Domain.ValueObjects.RoiValidation.RoiValidationConditionNode, string, Result>
            evaluateChild);
}
