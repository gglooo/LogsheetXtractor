    ...
    IMapper mapper) : IScriptInputPreparer
{
    ...
    public async Task<string> CreateAlignmentArgumentAsync(Logsheet logsheet, CancellationToken ct)
    {
        ...
        if (frontsidePoints == null || frontsidePoints.Count == 0)
        {
            return "--aligned";
        }

        var w = logsheet.Template.Width ?? 0;
        var h = logsheet.Template.Height ?? 0;
        if (w == 0 || h == 0)
        {
        ...
        var alignmentConfig = new PythonAlignmentConfig(templateCorners, frontsidePoints);
