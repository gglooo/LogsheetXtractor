...
namespace WebFormHTR.Tests.Application.Features.RoiValidation;
...
public class GetPredefinedRoiValidationConditionsTests : IDisposable
{
    ...
    public async Task Handle_ShouldReturnAllPredefinedConditions_WhenNoRoiTypeFilter()
    {
        ...
        var result = await GetPredefinedRoiValidationConditionsHandler.Handle(query, _dbContext, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.Should().HaveCount(3);
        result.Value.Select(x => x.Code).Should().Contain(new[] { "year", "month", "barcode-prefix" });
    }
                ...
                Condition = new RoiValidationConditionNode
                {
                    ...
                    [
                        new RoiValidationConditionNode
                        {
                            ...
                            Params = System.Text.Json.JsonSerializer.SerializeToElement(new { prefix = "ABC" })
                        }
                    ]
                }
            });
