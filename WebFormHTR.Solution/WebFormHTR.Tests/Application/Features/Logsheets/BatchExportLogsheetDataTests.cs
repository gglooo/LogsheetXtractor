...
public class BatchExportLogsheetDataTests
{
    ...
    public async Task Handle_ShouldReturnFile_WhenExportSucceeds()
    {
        ...
        var expectedFile = new GetFileDto { Stream = new MemoryStream(), FileName = "batch_export.zip", ContentType = "application/zip" };

        _logsheetExportServiceMock.Setup(x => x.ExportBatchLogsheetDataAsync(logsheetIds, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Ok(expectedFile));

        var result = await BatchExportLogsheetDataHandler.Handle(command, _logsheetExportServiceMock.Object, CancellationToken.None);
