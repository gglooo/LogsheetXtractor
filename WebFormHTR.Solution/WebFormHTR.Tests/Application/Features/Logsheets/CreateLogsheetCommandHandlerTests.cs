...
public class CreateLogsheetCommandHandlerTests : IDisposable
{
    ...
    public async Task Handle_ShouldCreateAndPublishEvent_WhenCreatedLogsheetIsPending()
    {
        ...
        var template = new Domain.Entities.Template { Id = templateId, Name = "Template", FileId = templateFile.Id };
        var file = new Domain.Entities.File { Id = fileId, OriginalFileName = "logsheet.jpg", StoragePath = "path" };
        ...
        var expectedDto = new LogsheetDetailDto(
            mappedLogsheet.Id,
            new TemplateListDto(templateId, "Template", null, null, null, 0, 0, 0, 0, DateTime.UtcNow),
        ...
        _mapperMock.Setup(m => m.Map<LogsheetDetailDto>(It.Is<Logsheet>(l => l.Id == mappedLogsheet.Id)))
            .Returns(expectedDto);
    ...
    public async Task Handle_ShouldCreateAndPublishEvent_WhenCreatedLogsheetIsNotPending()
    {
        ...
        _mapperMock.Setup(m => m.Map<Logsheet>(command)).Returns(mappedLogsheet);
        _mapperMock.Setup(m => m.Map<LogsheetDetailDto>(It.IsAny<Logsheet>()))
            .Returns(new LogsheetDetailDto(
                mappedLogsheet.Id,
                new TemplateListDto(templateId, "Template", null, null, null, 0, 0, 0, 0, DateTime.UtcNow),
                new FileDto(fileId, "logsheet.jpg", "image/jpeg", 100, DateTime.UtcNow),
    ...
    public async Task Handle_ShouldFail_WhenFileNotFound()
    {
        ...
        _busMock.Verify(b => b.PublishAsync(It.IsAny<LogsheetCreatedEvent>(), It.IsAny<DeliveryOptions>()),
            Times.Never);
    }
    ...
    {
        var fileId = Guid.NewGuid();
        _dbContext.Files.Add(new Domain.Entities.File { Id = fileId, OriginalFileName = "f.jpg", StoragePath = "p" });
        await _dbContext.SaveChangesAsync();
